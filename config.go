...
type Config struct {
	...
	DiscardSenderQueueOnHTTPResponseError bool `toml:"discard_sender_queue_on_http_response_error" comment:"Discard unsent data from the local queue of the current sender thread\nif the hub responds with a status code other than 2XX."`
	DiscardSenderQueueOnHTTPConnectError  bool `toml:"discard_sender_queue_on_http_connect_error" comment:"Discard unsent data from the local queue of the current sender thread\nif the hub connection times out, name resolution fails,\nor cannot be established for other reasons."`

	MaxUnsentQueueSize int `toml:"max_unsent_queue_size" comment:"Keep this maximum number of check results in the local queue.\nOldest data will be deleted first.\nIgnored if queues are discarded on errors. See above."`
...
func NewConfig() *Config {
	cfg := &Config{
		...
		DiscardSenderQueueOnHTTPResponseError: true,
		DiscardSenderQueueOnHTTPConnectError:  true,
		MaxUnsentQueueSize:                    100,

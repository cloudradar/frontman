...
func (tb PagesToolBar) Create(builder *Builder) error {
	w, err := walk.NewToolBarWithOrientationAndButtonStyle(
		...
		walk.ToolBarButtonStyle(tb.ButtonStyle),
	)
	if err != nil {
		return err
	}

	if tb.ToolBar.AssignTo != nil {
		*tb.ToolBar.AssignTo = w
	}
	if tb.IconSize.Height == 0 {
		tb.IconSize.Height = 16
	}
	if tb.IconSize.Width == 0 {
		tb.IconSize.Width = 16
	}

	return builder.InitWidget(tb, w, func() error {
		imageList, err := walk.NewImageList(tb.IconSize, 0)
		if err != nil {
			return err
		}

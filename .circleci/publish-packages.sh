...
github_upload() {
  github-release upload --user cloudradar-monitoring --repo frontman --tag ${CIRCLE_TAG} "$@"
}
...
# scan signed MSI
virustotal-scan --ignore Cylance,Jiangmin,Ikarus,MaxSecure --apikey ${VIRUSTOTAL_TOKEN} --file ${PROJECT_DIR}/dist/frontman_64.msi
...
echo -e ${CHANGELOGRAW} | sed -e 's/^"//' -e 's/"$//' | github-release edit --user cloudradar-monitoring --repo frontman --tag ${CIRCLE_TAG} ${PRERELEASE} --description -
